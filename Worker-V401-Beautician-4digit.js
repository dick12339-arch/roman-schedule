// roman worker v401 beautician 4digit - lower case files
...
function parseSchedule(text){
  ...
  for(let block of blocks){
    ...
    let mDash = first.match(/^(.+?)(\d{1,2}(?:[:：]\d{1,2})?|\d{3,4})\s*[-—~至]+\s*(\d{1,2}(?:[:：]\d{1,2})?|\d{3,4})(.*)$/);
    if(mDash){
      name = mDash[1].trim();
    ...
    for(let i=1;i<lines.length;i++){
      ...
      if(!line) continue;
      if(line.includes('不排')){
        let bmTime = line.match(/^(\d{1,2}(?:[:：]\d{1,2})?|\d{3,4})\s*(.+)?$/);
      ...
      let bm = line.match(/^(\d{1,2}(?:[:：]\d{1,2})?|\d{3,4})\s*(.*)$/);
      if(bm){
  ...
  async fetch(request, env, ctx){
    ...
    if(url.pathname==='/api/schedule' && request.method==='POST'){
      try{
        ...
        await insertSchedule(env.ROMAN_DB, id, raw_text, body.updated_by||'v401_beautician_4digit', body.source||'v401_beautician_4digit');
        ...
        for(let ev of events){
          if(ev.type==='message' && ev.message.type==='text'){
            ...
            if(text.length > 20 && (text.includes('太原') || text.includes('日進') || text.includes('日新') || text.includes('-') || text.includes('---------------') || text.includes('不排'))){
              ...
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){ try{ await fetch('https://api.line.me/v2/bot/message/reply',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+env.LINE_CHANNEL_TOKEN},body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (美容師版 4位數支援)\n${text.split('\n')[0]}...`} ]})}); }catch{} }
            }
          }
        }
    ...
    return new Response('roman worker v401 beautician 4digit ok',{headers:cors});
  }
};

// roman worker v401 beautician 4digit - fix 1030 as time not customer - 1030 = 10:30
  ...
  async fetch(request, env, ctx){
    ...
    if(url.pathname==='/api/schedule' && request.method==='POST'){
      try{
        ...
        await insertSchedule(env.ROMAN_DB, id, raw_text, body.updated_by||'v401_beautician_1030fix', body.source||'v401_beautician_1030fix');
        ...
        for(let ev of events){
          if(ev.type==='message' && ev.message.type==='text'){
            ...
            if(text.length > 20 && (text.includes('太原') || text.includes('日進') || text.includes('日新') || text.includes('-') || text.includes('---------------') || text.includes('不排'))){
              ...
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){ try{ await fetch('https://api.line.me/v2/bot/message/reply',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+env.LINE_CHANNEL_TOKEN},body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (美容師版 1030修正)\n${text.split('\n')[0]}...`} ]})}); }catch{} }
            }
          }
        }
    ...
    return new Response('roman worker v401 beautician 1030 fix ok',{headers:cors});
  }
};

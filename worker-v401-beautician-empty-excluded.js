// Roman Worker v401 - 美容師版・空檔+不排不計入預約數 + 三版型
...
<div class="container" style="padding-top:8px;padding-bottom:0">
  <div class="version-bar">
    <span>v401-美容師・空檔不排不計入預約・電腦/直式/橫式獨立佈局・按鈕不重疊・數值不被遮・面板可關閉</span>
  ...
  display.innerHTML = filtered.map(t=>{
    ...
    return \`<div class="teacher-card">
      ...
      <div style="width:100%;max-width:100%">
        \${t.bookings.length===0 ? \`<div style="text-align:center;color:#a1a1aa;padding:12px;border:1px dashed #e4e4e7;border-radius:10px;font-size:12px">\${t.isBuPaiHeader ? '今日不排休' : '今日無預約'}</div>\` : t.bookings.map(b=>{
          // 不排：獨立樣式，不列入預約數
          if(b.isBuPai){
...
export default {
  async fetch(request, env, ctx){
        ...
        for(let ev of events){
          if(ev.type==='message' && ev.message.type==='text'){
            ...
            if(text.length > 20 && (text.includes('太原') || text.includes('日進') || text.includes('日新') || text.includes('-') || text.includes('---------------') || text.includes('不排'))){
              ...
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){ try{ await fetch('https://api.line.me/v2/bot/message/reply',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+env.LINE_CHANNEL_TOKEN},body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (美容師版・空檔/不排不計入)\n${text.split('\n')[0]}...`} ]})}); }catch{} }
            }
          }
        }
    ...
    return new Response('Roman Worker v401 Beautician 空檔不計入 OK',{headers:cors});
  }
};

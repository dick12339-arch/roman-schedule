// roman worker v401 beautician 4digit + empty card + 24h fix - lower case
...
.booking-bupai{background:#f4f4f5;color:#71717a;border:1px solid #e4e4e7;font-style:italic}
.booking-match-time{animation: pulse 1.2s ease-in-out 2}
  ...
  <div class="header-inner">
    <div style="display:flex;flex-direction:column;gap:6px;min-width:0;flex:1">
      <div class="title">
        ...
        <span class="badge badge-amber">v401 美容師版・不排排除</span>
        <span class="badge badge-blue">LINE + D1</span>
      </div>
      <div style="font-size:11px;color:#71717a;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">接龍制・22位美容師・日進 / 太原 / 日新 / 忠誠 / 大連 / 南屯・不等比縮小</div>
    </div>
...
<div class="container" style="padding-top:8px;padding-bottom:0">
  <div class="version-bar">
    <span>v401-美容師・空檔時間搜尋・空檔不排不計入・電腦/直式/橫式獨立佈局・按鈕不重疊・數值不被遮・面板可關閉</span>
    ...
    <div class="right-col">
      <div class="card" style="margin-bottom:14px">
        <div style="position:relative;margin-bottom:10px">
          <input id="searchInput" class="input" type="text" placeholder="搜尋美容師 / 客戶">
          <span style="position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#a1a1aa;pointer-events:none">🔍</span>
        </div>
        <div style="position:relative;margin-bottom:12px">
          <input id="timeSearchInput" class="input" type="text" placeholder="搜尋空檔時間 (例:10、11、14)" inputmode="numeric">
          <span style="position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#a1a1aa;pointer-events:none">⏰</span>
          <div id="timeSearchHint" style="font-size:10px;color:#71717a;margin-top:6px;display:none"></div>
        </div>
        ...
        <div class="stats">
          <div class="stat"><div class="stat-label">美容師</div><div id="statTeachers" class="stat-value">0</div></div>
      ...
      <div id="emptyCard" class="card" style="margin-bottom:14px;display:none">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;gap:8px">
          <div style="font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px">
            <span>📋 空檔美容師</span>
            <span id="emptyCardCount" class="badge badge-emerald">0 位</span>
            <span id="emptyCardTime" class="badge badge-zinc" style="font-size:10px"></span>
          </div>
          <button id="btnCopyEmpty" class="btn" style="height:32px;padding:0 14px;font-size:12px;background:#18181b;color:#fff;border-radius:999px;width:auto;min-width:80px;flex-shrink:0">📋 複製</button>
        </div>
        <textarea id="emptyListTextarea" class="textarea" style="height:140px;font-size:13px;line-height:1.8;background:#f8fafc;white-space:pre;overflow-y:auto" readonly placeholder="輸入空檔時間後，這裡會顯示&#10;昀蓁12&#10;花花12&#10;&#10;可直接複製貼到 LINE"></textarea>
        <div id="emptyCardHint" style="font-size:10px;color:#71717a;margin-top:6px;display:flex;justify-content:space-between">
          <span>格式：美容師+時間，可直接貼到 LINE</span>
          <span id="emptyCardStatus"></span>
        </div>
      </div>
...
function parseTimeSearch(input){
  if(!input) return [];
  // 支援 10、11、14 或 10,11,14 或 10 11 或 10-12 這種
  ...
  for(let p of parts){
    ...
    if(p.includes('-') || p.includes('—') || p.includes('~')){
      ...
      const [a,b] = p.split(sep).map(x=>parseInt(x.trim()));
      if(!isNaN(a) && !isNaN(b)){
        for(let h=Math.min(a,b); h<=Math.max(a,b); h++) if(h>=0 && h<=24) hours.add(h);
      }
    }else{
      const h = parseInt(p);
      if(!isNaN(h) && h>=0 && h<=24) hours.add(h);
    }
  }
...
function parseTimeToken(token){
  ...
  if(token.includes(':') || token.includes('：')){
    ...
    return {hour: h, minute: isNaN(m)?0:m, raw: token};
  }
  if(/^\d{3,4}$/.test(token)){
    if(token.length===3){
      const h = parseInt(token[0]);
      const m = parseInt(token.slice(1));
      return {hour: h, minute: m, raw: token};
    }else{
  ...
  if(/^\d{1,2}$/.test(token)){
    ...
    return {hour: h, minute: 0, raw: token};
  }
  return null;
}
...
function formatHour(h, m){
  if(h==null) return '—';
  if(m && m!==0) return \`\${h}:\${String(m).padStart(2,'0')}\`;
  return \`\${h}:00\`;
}
function formatRange(s, sm, e, em){
  if(s==null || e==null) return '';
  const sStr = (sm && sm!==0) ? \`\${s}:\${String(sm).padStart(2,'0')}\` : \`\${s}\`;
  const eStr = (em && em!==0) ? \`\${e}:\${String(em).padStart(2,'0')}\` : \`\${e}\`;
  return \`\${sStr}-\${eStr}\`;
}
...
function parseSchedule(text){
  ...
  for(let block of blocks){
    ...
    const first = lines[0];
    const isHeaderBuPai = first.includes('不排');
    let name = '';
    let start = null, startMin = 0;
    let end = null, endMin = 0;
    let location = '';
    let parsedHeader = false;

    let mDash = first.match(/^(.+?)(\d{3,4}|\d{1,2}(?:[:：]\d{1,2})?)\s*[-—~至]+\s*(\d{3,4}|\d{1,2}(?:[:：]\d{1,2})?)(.*)$/);
    if(mDash){
      name = mDash[1].trim();
      const sTok = mDash[2].trim();
      const eTok = mDash[3].trim();
      location = (mDash[4]||'').trim();
      const s = parseTimeToken(sTok);
      const e = parseTimeToken(eTok);
      if(s && e){
        start = s.hour; startMin = s.minute;
        end = e.hour; endMin = e.minute;
        parsedHeader = true;
      }
    }

    if(!parsedHeader){
      let mConcat = first.match(/^(.+?)(\d{3,4})(.*)$/);
      if(mConcat){
        const maybe = mConcat[2];
        if(maybe.length===4){
          const sH = parseInt(maybe.slice(0,2));
          const eH = parseInt(maybe.slice(2));
          if(sH>=0 && sH<=24 && eH>=0 && eH<=24){
            const tail = parseInt(maybe.slice(2));
            const isCommonMinute = (tail===0 || tail===30);
            const isLikelyTime = (tail<60 && isCommonMinute);
            if(!isLikelyTime){
              name = mConcat[1].trim();
              location = (mConcat[3]||'').trim();
              start = sH; startMin = 0;
              end = eH; endMin = 0;
              parsedHeader = true;
            }else{
              // 可能是 1030 這種單一時間，嘗試當作只有開始時間
              const t = parseTimeToken(maybe);
              if(t){
                name = mConcat[1].trim();
                location = (mConcat[3]||'').trim();
                start = t.hour; startMin = t.minute;
                end = null;
                parsedHeader = true;
              }
            }
          ...
          const sH = parseInt(maybe[0]);
          const eH = parseInt(maybe.slice(1));
          if(sH>=0 && sH<=24 && eH>=0 && eH<=24){
            name = mConcat[1].trim();
            location = (mConcat[3]||'').trim();
            start = sH; startMin = 0;
    ...
    if(!parsedHeader){
      let mOld = first.match(/^(.+?)(\d{1,2})(?:[:：]?\d{0,2})?[-—](\d{1,2})(.+)?$/);
      if(mOld){
        name = mOld[1].trim();
        start = parseInt(mOld[2]);
        end = parseInt(mOld[3]);
        location = (mOld[4]||'').trim();
        parsedHeader = true;
      }
    }
    ...
    if(!parsedHeader){
      if(isHeaderBuPai){
        let nameOnly = first.split('不排')[0].trim();
        nameOnly = nameOnly.replace(/\d+.*$/, '').trim();
        if(!nameOnly) nameOnly = first.replace('不排','').trim().split(/\s+/)[0] || '未命名美容師';
        result.push({name: nameOnly, start: null, end: null, startMin:0, endMin:0, location: '不排', bookings: [], raw: block, isBuPaiHeader: true});
      }
      continue;
    }

    const isLocationBuPai = location.includes('不排');
    const bookings = [];
    for(let i=1;i<lines.length;i++){
      const line = lines[i];
      if(!line) continue;
      if(line.includes('不排')){
        let bmTime = line.match(/^(\d{3,4}|\d{1,2}(?:[:：]\d{1,2})?)\s*(.+)?$/);
        let hour = null, minute = 0, customer = line;
        if(bmTime){
          const t = parseTimeToken(bmTime[1]);
          if(t){ hour = t.hour; minute = t.minute; customer = (bmTime[2]||line).trim() || line; }
        }
        bookings.push({hour, minute, customer, isBuPai: true, isExcluded: true});
        continue;
      }
      let bm = line.match(/^(\d{3,4}|\d{1,2}(?:[:：]\d{1,2})?)\s*(.*)$/);
      if(bm){
        const timeTok = bm[1].trim();
        const cust = (bm[2]||'').trim();
        const t = parseTimeToken(timeTok);
        if(t){
          bookings.push({hour: t.hour, minute: t.minute, customer: cust, isBuPai: false, isExcluded: false});
          continue;
        }
      }
      bookings.push({hour: null, minute: 0, customer: line, isBuPai: false, isExcluded: false});
    }
    result.push({name, start, end, startMin: startMin||0, endMin: endMin||0, location, bookings, raw: block, isBuPaiHeader: isHeaderBuPai || isLocationBuPai});
  }
  return result;
}
...
function renderSchedule(){
  ...
  // 新增：搜尋空檔時間，例如輸入 10、11 抓出該時段空檔的美容師
  const wantedHours = parseTimeSearch(timeSearchQuery);
  if(wantedHours.length>0){
    filtered = filtered.filter(d => d.bookings.some(b => 
      b.hour!=null && wantedHours.includes(b.hour) && !b.isBuPai && !b.isExcluded && (!b.customer || !b.customer.trim())
    ));
  }
  document.getElementById('statTeachers').textContent = filtered.length;
  // 空檔欄位不列入預約計算數 + 不排不列入 - 只算有客戶名的有效預約
  const allBookings = filtered.reduce((a,c)=>a+c.bookings.filter(b=>!b.isBuPai && !b.isExcluded && b.customer && b.customer.trim() && !b.customer.includes('不排')).length,0);
  document.getElementById('statBookings').textContent = allBookings;
  // 空檔統計：空字串且不是不排
  const emptyCount = filtered.reduce((a,c)=>a+c.bookings.filter(b=>!b.isBuPai && !b.isExcluded && (!b.customer||!b.customer.trim())).length,0);
  document.getElementById('statEmpty').textContent = emptyCount;
  document.getElementById('tabCount').textContent = filtered.length + ' 位美容師';

  // 新增：空檔美容師統計卡片，格式 昀蓁12 / 花花12
  const emptyCard = document.getElementById('emptyCard');
  const emptyTextarea = document.getElementById('emptyListTextarea');
  const emptyCountBadge = document.getElementById('emptyCardCount');
  const emptyTimeBadge = document.getElementById('emptyCardTime');
  if(wantedHours.length>0 && emptyCard && emptyTextarea){
    const emptyList = [];
    // 依美容師遍歷，抓出符合搜尋時間的空檔
    filtered.forEach(t=>{
      // 該美容師在此搜尋時段的空檔
      wantedHours.forEach(h=>{
        t.bookings.forEach(b=>{
          if(b.hour!=null && b.hour===h && !b.isBuPai && !b.isExcluded && (!b.customer || !b.customer.trim())){
            // 格式：美容師+時間，1030判讀為1030，不把30當客戶
            let timeStr = '';
            if(b.minute && b.minute!==0){
              timeStr = \`\${b.hour}\${String(b.minute).padStart(2,'0')}\`;
            }else{
              timeStr = \`\${b.hour}\`;
            }
            emptyList.push(\`\${t.name}\${timeStr}\`);
          }
        });
      });
    });
    // 去重並排序
    const uniqueList = [...new Set(emptyList)].sort();
    if(uniqueList.length>0){
      emptyTextarea.value = uniqueList.join('\n');
      if(emptyCountBadge) emptyCountBadge.textContent = \`\${uniqueList.length} 位\`;
      if(emptyTimeBadge) emptyTimeBadge.textContent = \`\${wantedHours.join('、')} 點空檔\`;
      emptyCard.style.display = 'block';
    }else{
      emptyTextarea.value = \`搜尋 \${wantedHours.join('、')} 點，沒有空檔美容師\`;
      if(emptyCountBadge) emptyCountBadge.textContent = '0 位';
      if(emptyTimeBadge) emptyTimeBadge.textContent = \`\${wantedHours.join('、')} 點\`;
      emptyCard.style.display = 'block';
    }
  }else{
  ...
  display.innerHTML = filtered.map(t=>{
    ...
    const timeBadge = (t.start!=null) ? \`<span class="badge badge-zinc" style="font-size:10px">\${formatRange(t.start, t.startMin, t.end, t.endMin)}</span>\` : '';
    // 每位美容師的有效預約數（空檔和不排不算）
    const validCount = t.bookings.filter(b=>!b.isBuPai && !b.isExcluded && b.customer && b.customer.trim() && !b.customer.includes('不排')).length;
    const timeText = (t.start!=null && t.end!=null) ? \`\${formatRange(t.start, t.startMin, t.end, t.endMin)}時\` : (t.isBuPaiHeader ? '不排休' : '無時段');
    return \`<div class="teacher-card">
      <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:12px;min-width:0">
        <div style="min-width:0;flex:1">
          ...
          <div style="font-size:11px;color:#71717a;margin-top:4px;font-family:monospace">\${validCount} 筆預約・\${timeText}</div>
        </div>
        <div style="width:36px;height:36px;border-radius:50%;background:\${validCount>0?'#18181b':'#e4e4e7'};color:\${validCount>0?'#fff':'#71717a'};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0;position:relative;z-index:1">\${validCount}</div>
      </div>
      <div style="width:100%;max-width:100%">
        \${t.bookings.length===0 ? \`<div style="text-align:center;color:#a1a1aa;padding:12px;border:1px dashed #e4e4e7;border-radius:10px;font-size:12px">\${t.isBuPaiHeader ? '今日不排休' : '今日無預約'}</div>\` : (()=>{ const wantedHoursInner = parseTimeSearch(timeSearchQuery); return t.bookings.map(b=>{
          // 不排：獨立樣式，不列入預約數
          if(b.isBuPai){
            return \`<div class="booking-row booking-bupai">
              <span class="mono" style="width:48px;flex-shrink:0;font-weight:600">\${formatHour(b.hour, b.minute)}</span>
              <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0">🚫 \${b.customer}</span>
              <span class="badge badge-zinc" style="font-size:9px;flex-shrink:0">不排</span>
            </div>\`;
          }
          const has = b.customer && b.customer.trim();
          const isEmpty = !has;
          const isMatchTime = isEmpty && wantedHoursInner.length>0 && b.hour!=null && wantedHoursInner.includes(b.hour);
          return \`<div class="booking-row \${has?'booking-filled':'booking-empty'} \${isMatchTime?'booking-match-time':''}" \${isMatchTime?'style="outline:2px solid #10b981;outline-offset:-2px;background:#ecfdf5;border-color:#10b981"':''}>
            <span class="mono" style="width:48px;flex-shrink:0;font-weight:600">\${formatHour(b.hour, b.minute)}</span>
            <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0">\${has?b.customer:'空檔'}</span>
            \${isMatchTime?'<span class="badge badge-emerald" style="font-size:9px;flex-shrink:0">符合</span>':''}
          </div>\`;
        }).join('') })()}
      </div>
    </div>\`;
...
async function init(){
  ...
  // 複製空檔美容師清單
  const btnCopyEmpty = document.getElementById('btnCopyEmpty');
  const emptyStatus = document.getElementById('emptyCardStatus');
  if(btnCopyEmpty){
    btnCopyEmpty.onclick = async ()=>{
      const ta = document.getElementById('emptyListTextarea');
      if(!ta || !ta.value) return;
      const orig = btnCopyEmpty.innerHTML;
      try{
        await navigator.clipboard.writeText(ta.value);
        btnCopyEmpty.innerHTML = '✅ 已複製';
        if(emptyStatus) emptyStatus.textContent = '已複製到剪貼簿';
        logSync(\`已複製空檔清單 \${ta.value.split('\n').length} 行\`, '系統');
        setTimeout(()=>{ btnCopyEmpty.innerHTML = orig; if(emptyStatus) emptyStatus.textContent=''; }, 2000);
      }catch{
        // fallback 選取
        ta.select();
        document.execCommand('copy');
        btnCopyEmpty.innerHTML = '✅ 已複製';
        setTimeout(()=>{ btnCopyEmpty.innerHTML = orig; }, 2000);
      }
    };
  }
  const timeInput = document.getElementById('timeSearchInput');
  const timeHint = document.getElementById('timeSearchHint');
  if(timeInput){
    timeInput.addEventListener('input', (e)=>{
      timeSearchQuery=e.target.value;
      const hours = parseTimeSearch(timeSearchQuery);
      if(timeHint){
        if(hours.length>0){
          timeHint.style.display='block';
          timeHint.textContent = \`🔍 正在篩選空檔：\${hours.map(h=>h+':00').join('、')}（\${hours.length}個時段）\`;
        }else if(timeSearchQuery.trim()){
          timeHint.style.display='block';
          timeHint.textContent = '請輸入 0-23 的小時，例如 10、11 或 10-12';
        }else{
    ...
    timeInput.addEventListener('keydown', (e)=>{
      if(e.key==='Escape'){ timeInput.value=''; timeSearchQuery=''; if(timeHint) timeHint.style.display='none'; renderSchedule(); }
    });
  }
  ...
  async fetch(request, env, ctx){
    ...
    if(url.pathname==='/api/schedule' && request.method==='GET'){
      try{
        ...
        try{ const r = await env.ROMAN_DB.prepare('SELECT * FROM roman_schedule ORDER BY updated_at DESC LIMIT 1').first(); if(r) return new Response(JSON.stringify({raw_text:r.raw_text, parsed: r.parsed ? JSON.parse(r.parsed) : [], updated_at:r.updated_at, updated_by:r.updated_by, source:r.source}),{headers:{...cors,'Content-Type':'application/json'}}); }catch(e){ const r = await env.ROMAN_DB.prepare('SELECT id, raw_text, updated_at, updated_by, source FROM roman_schedule ORDER BY updated_at DESC LIMIT 1').first(); if(r) return new Response(JSON.stringify({raw_text:r.raw_text, parsed:[], updated_at:r.updated_at, updated_by:r.updated_by, source:r.source}),{headers:{...cors,'Content-Type':'application/json'}}); }
    ...
    if(url.pathname==='/api/schedule' && request.method==='POST'){
      try{
        const body = await request.json(); const raw_text = body.raw_text || body.text || ''; if(!raw_text.trim()) return new Response(JSON.stringify({error:'raw_text required'}),{status:400, headers:cors});
        const id = 'web_' + Date.now(); if(!hasD1){ memoryStore = {raw_text, updated_at: new Date().toLocaleString('zh-TW'), updated_by:'memory'}; return new Response(JSON.stringify({success:true, id, hasD1:false}),{headers:{...cors,'Content-Type':'application/json'}}); }
        await insertSchedule(env.ROMAN_DB, id, raw_text, body.updated_by||'v401_beautician_24fix', body.source||'v401_beautician_24fix');
    ...
    if(url.pathname==='/webhook/line' && request.method==='POST'){
      try{
        const body = await request.json(); const events = body.events || [];
        for(let ev of events){
          if(ev.type==='message' && ev.message.type==='text'){
            const text = ev.message.text;
            if(text.length > 20 && (text.includes('太原') || text.includes('日進') || text.includes('日新') || text.includes('-') || text.includes('---------------') || text.includes('不排'))){
              const id = 'line_' + Date.now() + '_' + Math.random().toString(36).slice(2,6);
              if(hasD1){ await insertSchedule(env.ROMAN_DB, id, text, ev.source.userId || 'line_user', 'line_bot'); try{ await env.ROMAN_DB.prepare('INSERT INTO sync_logs (type, source, payload) VALUES (?, ?, ?)').bind('line_update', ev.source.userId||'line', text.slice(0,200)).run(); }catch{} }else{ memoryStore = {raw_text:text, updated_at: new Date().toLocaleString('zh-TW'), updated_by:'line'}; }
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){ try{ await fetch('https://api.line.me/v2/bot/message/reply',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+env.LINE_CHANNEL_TOKEN},body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (24h修正版)\n${text.split('\n')[0]}...`} ]})}); }catch{} }
            }
          }
        }
    ...
    if(url.pathname==='/' || url.pathname==='/index.html'){ return new Response(v401_html,{headers:{'Content-Type':'text/html; charset=utf-8', ...cors}}); }
    return new Response('roman worker v401 beautician 24h fix ok',{headers:cors});
  }
};

// Roman Worker v401 - 美容師版・空檔時間搜尋 + 空檔/不排不計入
...
function parseSchedule(text){
  ...
  for(let block of blocks){
    ...
    let m = first.match(/^(.+?)(\d{1,2})(?:[:：]?\d{0,2})?[-—](\d{1,2})(.+)?$/);
    // 標頭就是不排，沒有時間也保留美容師資料，預約數不計算
    if(!m){
      if(isHeaderBuPai){
        // 取不排前的名字當美容師名
        ...
        result.push({name: nameOnly, start: null, end: null, location: '不排', bookings: [], raw: block, isBuPaiHeader: true});
      }
      continue;
    }
    const name = m[1].trim();
    ...
    for(let i=1;i<lines.length;i++){
      ...
      if(line.includes('不排')){
        ...
        if(bm){
          hour = parseInt(bm[1]);
          customer = (bm[3]||line).trim() || line;
        }
        bookings.push({hour, customer, isBuPai: true, isExcluded: true});
        continue;
      }
      const bm = line.match(/^(\d{1,2})(?::?(\d{0,2}))?\s*(.+)?$/);
      if(bm){
        ...
        if(customer) bookings.push({hour, customer, isBuPai: false, isExcluded: false});
        else bookings.push({hour, customer: '', isBuPai: false, isExcluded: false});
      }else if(line){
        bookings.push({hour: null, customer: line, isBuPai: false, isExcluded: false});
      }
    }
    result.push({name, start, end, location, bookings, raw: block, isBuPaiHeader: isHeaderBuPai || isLocationBuPai});
  }
  return result;
}
...
function renderSchedule(){
  ...
  display.innerHTML = filtered.map(t=>{
    ...
    const timeText = (t.start!=null && t.end!=null) ? \`\${t.start}-\${t.end}時\` : (t.isBuPaiHeader ? '不排休' : '無時段');
  ...
  async fetch(request, env, ctx){
    ...
    if(url.pathname==='/api/schedule' && request.method==='POST'){
      try{
        ...
        await insertSchedule(env.ROMAN_DB, id, raw_text, body.updated_by||'v401_beautician', body.source||'v401_beautician');
        ...
        for(let ev of events){
          if(ev.type==='message' && ev.message.type==='text'){
            ...
            if(text.length > 20 && (text.includes('太原') || text.includes('日進') || text.includes('日新') || text.includes('-') || text.includes('---------------') || text.includes('不排'))){
              ...
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){ try{ await fetch('https://api.line.me/v2/bot/message/reply',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+env.LINE_CHANNEL_TOKEN},body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (美容師版)\n${text.split('\n')[0]}...`} ]})}); }catch{} }
            }
          }
        }
    ...
    return new Response('Roman Worker v401 Beautician 時間搜尋版 OK',{headers:cors});
  }
};

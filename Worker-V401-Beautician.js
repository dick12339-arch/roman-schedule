// Roman Worker v401 - 美容師版・不排不計算 + 三版型重排 + LINE + D1
// 新增邏輯：抓到「不排」不列入預約數，美容師名稱顯示
...
<div class="container" style="padding-top:8px;padding-bottom:0">
  <div class="version-bar">
    <span>v401-美容師・不排不計算・電腦/直式/橫式獨立佈局・按鈕不重疊・數值不被遮・面板可關閉</span>
...
export default {
  async fetch(request, env, ctx){
              ...
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){
                try{
                  await fetch('https://api.line.me/v2/bot/message/reply',{
                    ...
                    body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (美容師版・不排已排除計算)\n${text.split('\n')[0]}...`} ]})
                  });
                }catch{}
              }
            }
    ...
    return new Response('Roman Worker v401 Beautician OK - /api/schedule /webhook/line',{headers:cors});
  }
};

// roman worker v401 beautician secure - 24h + 1030 fix + empty card + security hardening
...
function getCorsHeaders(request){
  ...
  // allow if no origin (curl, server-to-server) or allowed list or github pages
  let allowOrigin = '*';
  if(origin){
...
async function verifyLineSignature(request, secret, bodyText){
  if(!secret) return true; // 若沒設 secret，暫時放行（建議上線後一定要設）
...
const SYNC_TOKEN = ''; // 前端可選：若 Worker 設了 SYNC_SECRET，這裡填同樣的 token 才可寫入
...
async function pushWorkerSchedule(raw_text){
  if(raw_text.length > 50000){ logSync('❌ 內容過大 (>50KB) 已阻擋', '資安'); alert('班表文字過大，請縮減後再同步'); return null; }
  const urls = [WORKER_URL + '/api/schedule', WORKER_BACKUP_URL + '/api/schedule'];
  const headers = {'Content-Type':'application/json'};
  if(SYNC_TOKEN) headers['Authorization'] = 'Bearer ' + SYNC_TOKEN;
  for(let url of urls){
    try{
      const res = await fetch(url, {method:'POST', headers, mode:'cors', body: JSON.stringify({raw_text, source:'roman_v401_responsive', updated_by:'roman_site'})});
      if(res.status===401){ logSync('❌ 未授權：SYNC_TOKEN 錯誤', '資安'); alert('未授權，請設定正確的 SYNC_TOKEN'); return null; }
      if(res.status===413){ logSync('❌ 內容過大被 Worker 拒絕', '資安'); continue; }
      const data = await res.json();
...
function renderSchedule(){
  ...
  display.innerHTML = filtered.map(t=>{
    const locBadge = t.location ? \`<span class="badge badge-dark" style="font-size:10px">\${escapeHtml(t.location)}</span>\` : '';
      ...
      <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:12px;min-width:0">
        <div style="min-width:0;flex:1">
          <div style="display:flex;flex-wrap:wrap;align-items:center;gap:8px;min-width:0">
            <h3 style="font-size:16px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%">\${escapeHtml(t.name)}</h3>
            \${locBadge}\${timeBadge}
          </div>
        ...
        \${t.bookings.length===0 ? \`<div style="text-align:center;color:#a1a1aa;padding:12px;border:1px dashed #e4e4e7;border-radius:10px;font-size:12px">\${t.isBuPaiHeader ? '今日不排休' : '今日無預約'}</div>\` : (()=>{ const wantedHoursInner = parseTimeSearch(timeSearchQuery); return t.bookings.map(b=>{
          ...
          if(b.isBuPai){
            return \`<div class="booking-row booking-bupai">
              ...
              <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0">🚫 \${escapeHtml(b.customer)}</span>
          ...
          return \`<div class="booking-row \${has?'booking-filled':'booking-empty'} \${isMatchTime?'booking-match-time':''}" \${isMatchTime?'style="outline:2px solid #10b981;outline-offset:-2px;background:#ecfdf5;border-color:#10b981"':''}>
            <span class="mono" style="width:48px;flex-shrink:0;font-weight:600">\${formatHour(b.hour, b.minute)}</span>
            <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0">\${has?escapeHtml(b.customer):'空檔'}</span>
...
export default {
  async fetch(request, env, ctx){
    ...
    // --- GET schedule (public read, but with CORS restriction) ---
    ...
    // --- POST schedule (write) - requires auth + size limit + rate-ish check ---
    if(url.pathname==='/api/schedule' && request.method==='POST'){
      try{
        ...
        if(raw_text.length > 50000){
          return new Response(JSON.stringify({error:'raw_text too large, max 50k'}),{status:413, headers:cors});
        }
        // Basic content sanity: must contain at least one location keyword or dash, otherwise reject obvious abuse
        if(raw_text.length < 10){
          return new Response(JSON.stringify({error:'too short'}),{status:400, headers:cors});
        }
        ...
        // SQL injection already prevented by bind, but still limit updated_by/source length
        const ub = String(body.updated_by||'v401_secure').slice(0,100);
        const src = String(body.source||'v401_secure').slice(0,100);
      ...
      try{
        ...
        for(let ev of events){
          if(ev.type==='message' && ev.message.type==='text'){
            ...
            // Keep original heuristic but also require length < 10000 to avoid DoS
              ...
              if(env.LINE_CHANNEL_TOKEN && ev.replyToken){
                try{
                  await fetch('https://api.line.me/v2/bot/message/reply',{
                    ...
                    body: JSON.stringify({replyToken: ev.replyToken, messages:[{type:'text', text: `✅ 已同步到 D1 ${text.length} 字 (安全版)\n${text.split('\n')[0]}...`} ]})
                  });
                }catch{}
              }
            }
    ...
    if(url.pathname==='/' || url.pathname==='/index.html'){
      const secHeaders = {...cors, 'Content-Type':'text/html; charset=utf-8', 'X-Content-Type-Options':'nosniff', 'X-Frame-Options':'DENY', 'Referrer-Policy':'strict-origin-when-cross-origin'};
      return new Response(v401_html,{headers:secHeaders});
    }

    return new Response('Roman Worker v401 Secure OK - /api/schedule /webhook/line',{headers:cors});
  }
};
